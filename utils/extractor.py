...
class VidExtractor_png:
    def __init__(self, *args, **kwargs):
        self.vid_path_list = kwargs.get("vid_paths", args[0])
    ...
    def extract(self):
        ...
        for vid_loc in tqdm(self.vid_path_list):
            ...
            vid_frames = []
            for frame in frames:
                # img = cv2.imread(os.path.join(vid_loc, frame))
                # img = cv2.resize(img, (224, 224))  # Resize image to 224x224
                # img = img / 255
                img = np.array(Image.open(os.path.join(vid_loc, frame)).resize((224,224)))
                vid_frames.append(img)
            vid_list.append(np.array(vid_frames))
        return vid_list
